<template>
  <div class="flex flex-col gap-6 h-full">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-800 tracking-wide">PENDING APPROVALS</h1>
        <p class="text-sm text-slate-500 mt-1">Review and act on submitted purchase requests</p>
      </div>
    </div>
...
const filters = reactive({ status: 'submitted', dateFrom: '', dateTo: '' })
const activeFilters = reactive({ status: 'submitted', dateFrom: '', dateTo: '' })
const filtersActive = computed(() => activeFilters.dateFrom || activeFilters.dateTo || activeFilters.status !== 'submitted')
