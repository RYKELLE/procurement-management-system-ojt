<template>
  <div class="flex flex-col gap-6 h-full">
        ...
        <table class="w-full text-sm">
          <thead class="bg-slate-100 border-b border-slate-200 sticky top-0">
            <tr>
              ...
              <th class="text-left px-6 py-4 font-bold text-slate-700 uppercase tracking-wide text-xs w-48">Assigned Role</th>
      ...
      <div class="bg-white border border-slate-800 w-full max-w-md px-8 py-7">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-500 uppercase tracking-wide">Name <span class="text-red-500">*</span></label>
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-500 uppercase tracking-wide">Email <span class="text-red-500">*</span></label>
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-500 uppercase tracking-wide">Password <span class="text-red-500">*</span></label>
            <input v-model="addModal.form.password" type="password" placeholder="Min. 8 characters"
              class="border border-slate-300 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-slate-500 transition"
              :class="{ 'border-red-400': addModal.errors.password }" @input="addModal.errors.password = ''" />
            <span v-if="addModal.errors.password" class="text-xs text-red-500">{{ addModal.errors.password }}</span>
          </div>

          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-500 uppercase tracking-wide">Role <span class="text-red-500">*</span></label>
    ...
    <div v-if="editModal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="closeEditModal">
      <div class="bg-white border border-slate-800 w-full max-w-lg px-8 py-7">
        ...
        <div class="flex flex-col gap-2 max-h-72 overflow-y-auto pr-1">
          <label v-for="permission in allPermissions" :key="permission"
            ...
            <input type="checkbox" :value="permission" v-model="editModal.selectedPermissions" class="w-4 h-4 accent-slate-800" />
            <span class="text-sm text-slate-700">{{ permission }}</span>
          </label>
        </div>

        <div v-if="editModal.errors.general" class="mt-4 bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3">
    ...
    <div v-if="deleteModal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="closeDeleteModal">
      <div class="bg-white border border-slate-800 w-full max-w-sm px-8 py-7">
        ...
        <div v-if="deleteModal.error" class="mb-4 bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3">
          {{ deleteModal.error }}
        </div>
...
async function handleRoleChange(user) {
  try {
    // TODO: await api.patch(`/users/${user.id}/role`, { role: user.role })
